...
const jobs = [
  { id: 'thief', name: '盜賊', skills: ['奇襲', '隱身', '毒陷阱', '螺旋匕首', '投擲炸彈'] },
  { id: 'fighter', name: '格鬥家', skills: ['後退步', '爆裂拳：第1擊', '蓄力拳', '空翻踢：第1擊', '重踏踢'] },
  { id: 'dual-blades', name: '雙刀客', skills: ['雙重新月', '滑行狂怒', '怒號疾風', '旋轉突襲', '分裂斬'] }
];
...
test('三個可分享 route 呈現各 5 筆技能與資料限制', async ({ page }) => {
  for (const job of jobs) {
    ...
    expect(names).toEqual(job.skills);
...
test('公開 JSON 保持三職業、15 技能、route 與 changelog 契約', async ({ page }) => {
  ...
  const data = await page.evaluate(async () => {
    const [professions, professionSkillsRaw, changelog] = await Promise.all([
    ...
    return { professions, professionSkillsRaw, professionSkills: JSON.parse(professionSkillsRaw), changelog };
  });
